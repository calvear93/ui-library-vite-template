...
- Lowercase, hyphenated, noun-led, behavior-oriented: `user-export`, `auth-session`,
  `order-ingestion`. One capability = one coherent area of behavior, not one component.
...
- Keep it implementation-free: behavior, contracts, constraints — no component names or
  libraries (those belong in `design.md`).
